...
import { Download, Building2, UserMinus, CheckCircle, Loader2 } from 'lucide-react';
...
interface RefundConfirmationLetterProps {
  refund: RefundData;
  open: boolean;
  onClose: () => void;
}

export const RefundConfirmationLetter = ({ refund, open, onClose }: RefundConfirmationLetterProps) => {
  ...
  const { layoutSettings } = useSignatureLayout('refund');
  const { template, parseText, getBadgeColorClass, getBadgeIconColorClass } = useLetterTemplate('refund');
  ...
  useEffect(() => {
    ...
    if (open && refund.id) {
      getExistingLetterNumber(refund.id, 'resignation').then(num => {
        setLetterNumber(num);
      });
    ...
    try {
      ...
      if (!currentLetterNumber) {
        currentLetterNumber = await issueLetterNumber(
          ...
          refund.memberName,
          refund.memberNumber,
          { totalRefund: refund.totalRefund, exitDate: refund.exitDate }
        );
      ...
      link.download = `surat-pengembalian-dana-${currentLetterNumber?.replace(/\//g, '-') || refund.id}-${new Date().getTime()}.png`;
  ...
  const hasOutstandingLoan = refund.loanOutstanding > 0;
  const netRefund = refund.totalSavings - refund.loanOutstanding;

  // Template data for parsing variables
  const templateData: import('@/lib/templateVariables').TemplateData = {
    member: {
      name: refund.memberName,
      memberNumber: refund.memberNumber || '',
    },
    savings: {
      principal: refund.simpananPokok,
      mandatory: refund.simpananWajib,
      voluntary: refund.simpananSukarela,
      total: refund.totalSavings,
    },
    resignation: {
      date: refund.exitDate,
      totalRefund: refund.totalRefund,
      loanDeduction: refund.loanOutstanding,
    },
    cooperative: {
  ...
  const letterTitle = template?.title || t('Surat Pengembalian Dana Simpanan', 'Savings Refund Letter');
  const openingText = template?.opening_text 
    ? parseText(template.opening_text, templateData) 
    : t('Data Anggota yang Keluar:', 'Exiting Member Data:');
  ...
  const badgeText = template?.status_badge_text || t('Anggota Keluar/Non-Aktif', 'Member Exit/Inactive');
      ...
      <DialogContent className="max-w-lg p-0 gap-0 overflow-hidden">
        <DialogHeader className="p-4 pb-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            <span>{t('Surat Pengembalian Dana', 'Refund Confirmation Letter')}</span>
          </DialogTitle>
        </DialogHeader>
          ...
          >
            ...
            <div className={`flex items-center gap-3 rounded-lg p-4 mb-6 border ${getBadgeColorClass(badgeColor)}`}>
              ...
              <div>
                ...
                <p className="text-sm opacity-90">
                  {t('Tanggal Keluar', 'Exit Date')}: {formatDate(refund.exitDate)}
                </p>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900">{refund.memberName}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">{t('Nomor Anggota', 'Member Number')}</span>
                  <span className="font-semibold text-gray-900">{refund.memberNumber}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              <p className="text-sm font-semibold text-gray-700 mb-2">
                {t('Rincian Simpanan:', 'Savings Breakdown:')}
              </p>
                ...
                <table className="w-full text-sm">
                  <tbody>
                    <tr className="border-b border-gray-200">
                      ...
                      <td className="px-4 py-2 font-semibold text-gray-900 text-right">{formatCurrency(refund.simpananPokok)}</td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Simpanan Wajib', 'Mandatory Savings')}</td>
                      <td className="px-4 py-2 font-semibold text-gray-900 text-right">{formatCurrency(refund.simpananWajib)}</td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Simpanan Sukarela', 'Voluntary Savings')}</td>
                      <td className="px-4 py-2 font-semibold text-gray-900 text-right">{formatCurrency(refund.simpananSukarela)}</td>
                    </tr>
                    <tr className="border-b border-gray-200 bg-blue-50">
                      <td className="px-4 py-2 font-semibold text-blue-800">{t('Total Simpanan', 'Total Savings')}</td>
                      <td className="px-4 py-2 font-bold text-blue-900 text-right">{formatCurrency(refund.totalSavings)}</td>
                    </tr>
                    {hasOutstandingLoan && (
                      <tr className="border-b border-gray-200 bg-red-50">
                        <td className="px-4 py-2 font-semibold text-red-800">{t('Potongan Sisa Pinjaman', 'Loan Deduction')}</td>
                        <td className="px-4 py-2 font-bold text-red-600 text-right">-{formatCurrency(refund.loanOutstanding)}</td>
                      </tr>
                    )}
                    <tr className="bg-green-50">
                      <td className="px-4 py-3 font-bold text-green-800">{t('Total Pengembalian', 'Total Refund')}</td>
                      <td className="px-4 py-3 font-bold text-green-700 text-right text-lg">{formatCurrency(Math.max(0, netRefund))}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* Outstanding Notice */}
            {hasOutstandingLoan && netRefund < 0 && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <p className="text-sm text-red-800 font-semibold">
                  {t('Kewajiban yang Harus Dilunasi:', 'Outstanding Obligation:')}
                </p>
                <p className="text-lg font-bold text-red-700 mt-1">
                  {formatCurrency(Math.abs(netRefund))}
                </p>
                <p className="text-xs text-red-600 mt-2">
                  {t('Sisa simpanan tidak mencukupi untuk menutup sisa pinjaman. Anggota wajib melunasi kekurangan tersebut.', 
                     'Remaining savings are insufficient to cover outstanding loan. Member must pay the remaining balance.')}
                </p>
              </div>
            )}
              ...
              <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                ...
                <div>
                  <p className="font-semibold text-green-800">
                    {t('Dana Telah Dikembalikan', 'Funds Have Been Refunded')}
                  </p>
                  <p className="text-sm text-green-700">
                    {t('Tanggal Pengembalian', 'Refund Date')}: {formatDate(refund.refundDate)}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex justify-between items-end">
                ...
                {showRecipientSignature && (
                  <div className="text-center">
                    <p className="text-sm text-gray-600">{t('Penerima Dana', 'Fund Recipient')}</p>
                    <div className="h-14 mt-1 mb-1 border-b border-gray-400 w-28"></div>
                    <p className="text-xs font-semibold text-gray-700">{refund.memberName}</p>
                  </div>
                )}
        ...
        <div className="p-4 pt-0">
          <Button onClick={handleDownload} disabled={isGenerating} className="w-full">
            ...
            {isGenerating ? t('Memproses...', 'Processing...') : t('Unduh Surat Pengembalian', 'Download Refund Letter')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
