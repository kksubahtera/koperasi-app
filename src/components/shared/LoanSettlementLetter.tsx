...
import { Download, Building2, FileText, CheckCircle2, Loader2 } from 'lucide-react';
...
interface LoanSettlementLetterProps {
  loan: LoanSettlementData;
  open: boolean;
  onClose: () => void;
}

export const LoanSettlementLetter = ({ loan, open, onClose }: LoanSettlementLetterProps) => {
  ...
  const { layoutSettings } = useSignatureLayout('loan_settlement');
  const { template, parseText, getBadgeColorClass, getBadgeIconColorClass } = useLetterTemplate('loan_settlement');
  ...
  useEffect(() => {
    ...
    if (open && loan.id) {
      getExistingLetterNumber(loan.id, 'loan_settlement').then(num => {
        setLetterNumber(num);
      });
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!currentLetterNumber) {
        currentLetterNumber = await issueLetterNumber(
          ...
          { 
            ...
            totalPaid: loan.totalPaidPrincipal + loan.totalPaidInterest + loan.totalPaidPenalty,
            isEarlyPayoff: loan.isEarlyPayoff || false
          }
        );
      ...
      link.download = `surat-pelunasan-pinjaman-${currentLetterNumber?.replace(/\//g, '-') || loan.id}-${new Date().getTime()}.png`;
  ...
  const totalPaid = loan.totalPaidPrincipal + loan.totalPaidInterest + loan.totalPaidPenalty;
  const settlementDate = loan.completedDate || new Date().toISOString();
  ...
  const letterTitle = template?.title || t('Surat Keterangan Pelunasan Pinjaman', 'Loan Settlement Certificate');
  ...
  const closingText = template?.closing_text 
    ...
    : t('Demikian surat keterangan ini dibuat untuk dapat dipergunakan sebagaimana mestinya. Anggota yang bersangkutan telah melunasi seluruh kewajiban pinjamannya dan tidak memiliki tunggakan apapun kepada koperasi.', 'This certificate is issued for proper use. The member has settled all loan obligations and has no outstanding arrears with the cooperative.');
  ...
  const badgeText = template?.status_badge_text || (loan.isEarlyPayoff 
    ? t('Pinjaman Telah Dilunasi (Pelunasan Dini)', 'Loan Settled (Early Payoff)')
    : t('Pinjaman Telah Lunas', 'Loan Fully Settled'));
      ...
      <DialogContent className="max-w-lg p-0 gap-0 overflow-hidden">
        ...
        <div className="p-4 pt-2 overflow-auto max-h-[60vh]">
          ...
          >
            ...
            {/* Settlement Notice - Dynamic Badge */}
            <div className={`flex items-center gap-3 rounded-lg p-4 mb-6 border ${getBadgeColorClass(badgeColor)}`}>
              <CheckCircle2 className={`h-8 w-8 flex-shrink-0 ${getBadgeIconColorClass(badgeColor)}`} />
              <div>
                <p className="font-semibold">
                  {badgeText}
                </p>
                <p className="text-sm opacity-90">
                  {t('Tanggal Pelunasan', 'Settlement Date')}: {formatDate(settlementDate)}
                </p>
              </div>
            </div>
            ...
            <div className="mb-6">
              <p className="text-sm font-semibold text-gray-700 mb-2">
                {t('Rincian Pelunasan Pinjaman:', 'Loan Settlement Details:')}
              </p>
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <tbody>
                    <tr className="border-b border-gray-200">
                      <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Pokok Pinjaman Awal', 'Original Loan Principal')}</td>
                      <td className="px-4 py-2 font-semibold text-gray-900">{formatCurrency(loan.principalAmount)}</td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Jangka Waktu Awal', 'Original Term')}</td>
                      <td className="px-4 py-2 font-semibold text-gray-900">{loan.tenor} {t('Bulan', 'Months')}</td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Tanggal Pencairan', 'Disbursement Date')}</td>
                      <td className="px-4 py-2 font-semibold text-gray-900">{formatDate(loan.disbursementDate)}</td>
                    </tr>
                    <tr className="border-b border-gray-200 bg-blue-50">
                      <td className="px-4 py-2 text-blue-700">{t('Total Pokok Dibayar', 'Total Principal Paid')}</td>
                      <td className="px-4 py-2 font-semibold text-blue-800">{formatCurrency(loan.totalPaidPrincipal)}</td>
                    </tr>
                    <tr className="border-b border-gray-200">
                      <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Total Bunga Dibayar', 'Total Interest Paid')}</td>
                      <td className="px-4 py-2 font-semibold text-gray-900">{formatCurrency(loan.totalPaidInterest)}</td>
                    </tr>
                    {loan.totalPaidPenalty > 0 && (
                      <tr className="border-b border-gray-200">
                        <td className="px-4 py-2 bg-gray-50 text-gray-600">{t('Total Denda Dibayar', 'Total Penalty Paid')}</td>
                        <td className="px-4 py-2 font-semibold text-orange-600">{formatCurrency(loan.totalPaidPenalty)}</td>
                      </tr>
                    )}
                    <tr className="bg-green-50">
                      <td className="px-4 py-2 text-green-700 font-semibold">{t('Total Pembayaran', 'Total Payment')}</td>
                      <td className="px-4 py-2 font-bold text-green-800">{formatCurrency(totalPaid)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* Early Payoff Info */}
            {loan.isEarlyPayoff && loan.interestSaved && loan.interestSaved > 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <p className="text-sm text-blue-800">
                  <span className="font-semibold">{t('Penghematan Bunga', 'Interest Saved')}:</span>{' '}
                  {formatCurrency(loan.interestSaved)}
                </p>
                <p className="text-xs text-blue-700 mt-1">
                  {t('Anggota melakukan pelunasan dini sehingga mendapat penghematan bunga.', 
                     'Member completed early payoff and received interest savings.')}
                </p>
              </div>
            )}
            ...
            {/* Signature Section */}
            <div className="mt-8">
              <p className="text-sm text-gray-600 text-right mb-4">{formatDate(settlementDate)}</p>
              
              <div className="flex justify-between items-end">
                {/* Member signature - conditional */}
                {showRecipientSignature && (
                  <div className="text-center">
                    <p className="text-sm text-gray-600">{t('Peminjam', 'Borrower')}</p>
        ...
        <div className="p-4 pt-0">
          <Button onClick={handleDownload} disabled={isGenerating} className="w-full">
            ...
            {isGenerating ? t('Memproses...', 'Processing...') : t('Unduh Surat Pelunasan', 'Download Settlement Letter')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
